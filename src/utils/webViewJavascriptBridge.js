...
function CToast(msg, status) {
  if (isiOS) {
    setupWebViewJavascriptBridge((bridge) => {
      bridge.callHandler("suboNTTost", { msg: msg, duration: 2 }, function(
        response
      ) {});
    });
  }
...
function CLogout() {
  if (isiOS) {
    setupWebViewJavascriptBridge((bridge) => {
      bridge.callHandler('suboNTLogout', {},function(response) {});
    });
  }

  if (isAndroid) {
    window.JSBridge.exitLogin();
  }
}
export {
